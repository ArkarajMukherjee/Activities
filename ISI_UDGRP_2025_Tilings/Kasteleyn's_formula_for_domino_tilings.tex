...
\begin{frame}
    ...
    \begin{proof}
        ...
        As skew symmetric matrices are normal we can diagonalise them into $D:=Q^{\top}AQ=\operatorname{diag}(i\lambda_1,-i\lambda_1,\ldots)$ with $Q$ orthogonal. 
        Suppose wlog $\lambda_1\neq 0$. 
        Let $v=u+iv$ be a complex vector with $u,v$ being real, such that $Av=i\lambda v$ or, $Au+iAv=i\lambda u-\lambda v$ comparing the real and imaginary parts: $Au=-\lambda v,Av=\lambda u$. 
    \end{proof}
\end{frame}
\begin{frame}
    \begin{proof}[(continued)]
        ...
        $$[A]_{\mathcal B}^{\mathcal B}=\begin{bmatrix}0 & -\lambda\\ \lambda & 0\end{bmatrix}$$
...
\begin{frame}
    ...
    And using Muir's result we can link all of this back to the Pfaffian :
