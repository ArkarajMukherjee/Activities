...
\begin{frame}
    This is a presentation on Kasteleyn's formula [\cite{kasteleyn1961}]for the number of domino tilings of a $m\times n$ rectangle heavily based on [\cite{stucky2015}]. 
    The key result we prove is,
    \begin{theorem}[Kasteleyn, 1961]
        If $T(m,n)$ is the set of tilings of a $m\times n$ rectangle (wlog. $m$ is even) and $H(\pi),V(\pi)$ the number of horizontal and vertical dominos in $\pi\in T(m,n)$ respectively, then for any complex $x,y$ we have
        $$\sum_{\pi\in T(m,n)}x^{H(\pi)}y^{V(\pi)}=2^{\frac{mn}{2}}\prod_{k=1}^{m/2}\prod_{l=1}^{n}\qty(x^2\cos^2\frac{\pi k}{m+1}+y^2\cos^2\frac{\pi l}{n+1})^{\frac{1}{2}}$$
    \end{theorem}
    Setting $x=y=1$ gives us the number of such tilings.
\end{frame}
\begin{frame}
    ...
    First we will come up with a \textit{guess} $D'$, which only differs from $D$ by some signs but the hardest problem of this part is constructing $D$ itself, signs included.
    In the second part of the proof we will use kronecker products and other linear algebraic tools to analyse $D$ and finally calculate its determinant which is equivalent to the main problem.
\end{frame}
...
\section[Translation of the problem into counting permutations]{Translation into counting permutations}
...
\begin{frame}
    ...
    These hand-wavingly make sense because flipping a domino should be immaterial to the number of tilings so we fix the higher label(equivalently both) and also that switching \textit{pairs} around just replaces two dominos which should also be immaterial - for example, if $m=n=2$ we see that $1234$ and $3412$ correspond to the exact same tiling.
...
% =========================================
\section[The Pfaffian]{The Pfaffian}
...
\begin{frame}
    ...
    \begin{proof}
        ...
        As skew symmetric matrices are normal we can diagonalise them into $D:=Q^{\top}AQ=\operatorname{diag}(i\lambda_1,-i\lambda_1,\ldots)$ with $Q$ orthogonal and thus it is sufficient to show that $D$ is orthogonally similar to $H$. 
        Suppose wlog $\lambda_1\neq 0$. 
        Let $v=u+iv$ be a complex vector with $u,v$ being real, such that $Av=i\lambda v$ or, $Au+iAv=i\lambda u-\lambda v$ comparing the real and imaginary parts: $Au=-\lambda v,Av=\lambda u$. \renewcommand{\qedsymbol}{{}}
    \end{proof}
\end{frame}
\begin{frame}
    \begin{proof}[continued]
        It can be shown that $u,v$ are orthogonal and have the same norm, thus we normalise them to orthonormal vectors $u',v'$. 
        Let $W:=\operatorname{Span}\{u',v'\}$, then clearly $AW\subseteq W$, let $\mathcal B=\{u',v'\}$ be its basis and we have 
        $$\qty[A\vert_{W}]_{\mathcal B}^{\mathcal B}=\begin{bmatrix}0 & -\lambda\\ \lambda & 0\end{bmatrix}$$
        It can be shown that $AW^{\perp}\subseteq W^{\perp}$ and we can proceed doing this inductively on $W^{\perp}$. 
        After we are done, a suitable change of basis gives us $H$ and the change of basis matrix has all columns being real orthonormal vectors and is orthogonal.
    \end{proof}
    \begin{theorem}
        For any $2n\times 2n$ skew symmetric matrix $A$ and arbitrary matrix $B$ we have, $\operatorname{Pf}(BAB^\top)=\det(B)\operatorname{Pf}(A)$.
    \end{theorem}
\end{frame}
\begin{frame}
    ...
    \end{theorem}
    \begin{proof}
        It is known that $P\in\mathbb R[x_1,\ldots,x_m]$ and $P(\mathbb R^m)=\{0\}\implies P\equiv 0$ (the proof is left as an exercise. Hint: Try to induct on the degree of $P$).
        ...
        Then, $\operatorname{Pf}(A)^2=\operatorname{Pf}(H)^2\det(Q)^2=\det(A)$ with the last equality left as another exercise.
    \end{proof}
\end{frame}
...
\section[Constructing the matrix]{Constructing the matrix}

\begin{frame}
    We first try to get a glimpse at the possible structure of $D$, only then rectifying our guess to the final matrix $D$ which will give us $\operatorname{Pf}(D)=|T(m,n)|$. 
    The Pfaffian goes through all the permutations which \underline{could} correspond to some tiling, but we want a way to filter out the \textit{bad} permutations.
    A way to do this is by setting the entries $D_{i,j}$ (with $i<j$, i.e. in the upper triangular half) where $i,j$ can not be labels on a singular domino in some canonical permutation, to zero.
    Then the only non zero entries could be :
    \begin{enumerate}
        \item horizontal dominos $\rightarrow$ clearly the larger even label must lie exactly to the right of the smaller odd label, unless the odd label is on the rightmost edge, in which case it can not correspond to a domino and we set it to zero.
        \item vertical dominos $\rightarrow$ here, the even label is similarly always directly above the smaller odd one, unless this odd one is on the topmost edge.
    \end{enumerate}
\end{frame}

\begin{frame}
    Thus combining these with the definition of permutations we are interested in, we take the following guess at $D$, namely $D'$: the $(i,j)$-th entry ($i<j$ still, entries with $i>j$ are determined by skew symmetry) is nonzero if 
    \begin{itemize}
    ...
    $$\prod_{i=1}^{mn/2}D'_{\sigma(2i-1),\sigma(2i)}=
    \begin{cases}
...
\section[Representing the matrix using Kronecker products]{Representing the matrix with kronecker products}
...
this is clearly in block diagonal form as $I_n,\Lambda_n$ are both diagonal and of the same dimension. 
We will now try to figure out what $U_m^{-1}V_mU_m$ is.
\end{frame}

% =========================================
% SECTION 6: Evaluating U_m^-1 V_m U_m
% =========================================

\section[Evaluating $U_m^{-1}V_mU_m$]{Evaluating $U_m^{-1}V_mU_m$}
\begin{frame}
...
\begin{frame}
    ...
    with some trigonometry, what this tells us is that $V_mU_m=U_mJ_m$ where $J_m$ is the anti-diagonal identity matrix (what we get by flipping $I_m$ along the horizontal axis.) Thus, simplifying everything we get,
...
% =========================================
% SECTION 7: Manipulating the blocks
% =========================================
...
\begin{frame}
    ...
    Now, using Muir's result we can link all of this back to the Pfaffian :
    $$\operatorname{Pf}(D)^2=\det(D)=\det((U_n\otimes U_m)\tilde{\Lambda}(U_n\otimes U_m)^{-1})=\det\tilde{\Lambda}$$
\end{frame}
\begin{frame}
    With what we have done so far, we have
    \begin{align*} 
        \sum_{\pi\in T(m,n)}x^{H(\pi)}y^{V(\pi)} &= \operatorname{Pf}(D) \\ &= \sqrt{\det\tilde{\Lambda}} \\ &= 2^{mn/2}\prod_{k=1}^n\prod_{j=1}^{m/2}\qty(x^2\cos^2\frac{\pi j}{m+1}+y^2\cos^2\frac{\pi k}{n+1})^{\frac{1}{2}}
    \end{align*}
